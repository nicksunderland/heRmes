...
N <- as.matrix(rowSums(V))
V <- V / diag(V)
diag_names <- data.table(description = rownames(V))
...
rownames(V) <- c(MORE_LETTERS[1:length(diag_names)], "Num")
colnames(V) <- c(paste0(diag_names, " - ", MORE_LETTERS[1:length(diag_names)]), "Num")
Vmelt <- reshape2::melt(V) |> data.table::as.data.table()
names(Vmelt) <- c("ICD10_1", "ICD10_2", "value")

# Plot 
ggplot(Vmelt, aes(x = ICD10_1, y = ICD10_2, fill = value, label = round(value, 2))) +
  geom_tile(color = "white") +
  geom_text(size = 2, aes(color = grepl("Num", ICD10_1) | grepl("Num", ICD10_2)), show.legend = FALSE) + 
  scale_fill_gradient(low = "white", high = "blue", limits = c(0, 1)) + 
  scale_color_manual(values = c("TRUE" = "white", "FALSE" = "black")) +
  theme_minimal() +
  labs(x = "Diagnosed ICD10", y = "Diagnosed ICD10", title = "Co-occurrence of ICD10 codes", fill = "%") +
  coord_fixed() 
```
