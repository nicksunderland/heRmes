...
Phenotype the individuals with phenotype `PH1643` (heart failure syndrome) or `PH1646` (cardiomyopathy), excluding phenotypes `PH1637` (congenital heart disease) and `PH1636` (myocardial infarction). There can be multiple included or excluded phenotypes given in a list. If the phenotype IDs are named, these names are used as column names in the result. The overall result is given in the column `overall`, although this can be renamed by giving the `name` parameter. 
```{r phenotype}
